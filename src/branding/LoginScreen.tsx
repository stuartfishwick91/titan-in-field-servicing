...
export function LoginScreen({ onLogin, error, management = false, cloud = false, busy = false }: LoginScreenProps) {
    ...
    <main className="titan-login" style={{ backgroundImage: `url("${branding.loginBackground || `${assets}login-background.png`}")` }}>
      <section className="titan-login-panel" aria-label={management ? "Management sign in" : "Sign in"}>
        ...
        <form className="titan-signin-form" onSubmit={submit}>
          <label htmlFor="titan-login-name">{cloud ? "EMAIL" : "USERNAME"}</label>
          <div className="titan-login-input">
            <UserRound size={22} aria-hidden="true" />
            <input id="titan-login-name" value={name} onChange={(event) => setName(event.target.value)} placeholder={cloud ? "Enter your email address" : "Enter your full name"} type={cloud ? "email" : "text"} autoComplete="username" required />
          </div>
          <label htmlFor="titan-login-pin">PASSWORD</label>
          <div className="titan-login-input">
            <LockKeyhole size={21} aria-hidden="true" />
            <input id="titan-login-pin" value={pin} onChange={(event) => setPin(event.target.value)} placeholder={cloud ? "Enter your password" : "Enter your PIN"} type={showPin ? "text" : "password"} inputMode={cloud ? undefined : "numeric"} pattern={cloud ? undefined : "[0-9]{4}"} maxLength={cloud ? undefined : 4} autoComplete="current-password" required aria-describedby="titan-login-hint" />
          ...
          <div className="titan-login-options">
            ...
            <button type="button" onClick={() => setHelp(cloud ? "Ask the project owner to reset your app password from Supabase Authentication → Users." : "Please contact your administrator to reset your PIN. Email password recovery is not connected in this trial.")}>Forgot password?</button>
          </div>
          {error && <p className="titan-login-message error" role="alert">{error}</p>}
          <button className="titan-login-submit" type="submit" disabled={busy}><LogIn size={22} aria-hidden="true" /><span>{busy ? "SIGNING IN…" : "LOGIN"}</span></button>
          {!cloud && <><div className="titan-login-divider"><span>OR</span></div>
          ...
          <p id="titan-login-hint" className="titan-login-hint">{cloud ? "Shared trial: use your app email and password on both devices." : "Trial access: use your full name and four-digit PIN."}{management && <> <Link to="/employee">Employee sign in</Link></>}</p>
        </form>
